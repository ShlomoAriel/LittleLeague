var SeasonCtrl = function ($scope, $location, Seasons, CommonServices, TeamService, $rootScope, $state, $stateParams, $http) {
    ...
    $scope.addTeamToSeason = function () {
        ...
        $http.post('http://beta.redlionleague.com///api/Match/AddTeamToSeason', SeasonTeam).success(function (data) {
        ...
        $http.post('http://beta.redlionleague.com///api/Match/DeleteTeamFromSeason', seasonTeam).success(function () {
            CommonServices.getSeasons();
            $scope.deletedAlert();
