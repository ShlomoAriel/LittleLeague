var PlayerCtrl = function ($scope, CommonServices,PlayerService, Players, TeamService, $http) {
    ...
    $scope.register = function () {
        ...
        $http.post('http://localhost:55506///api/Match/PostPlayer', $scope.player).success(function () {
    ...
    $scope.testRegister = function () {
        $scope.isProcessing = true;
        $http.post('http://localhost:55506///api/Account/Register', $scope.test).success(function () {
            alert('נרשמת בהצלחה!');
        }).error(function (data) {
            alert('שגיאה! אולי טעות בקוד קבוצה');
        });
